# Sprint 4 Technical Documentation
...
### Sprint Goals
- Complete remaining Sprint 3 issues
- Add the logic to buy listings
...
- Added simple logic to purchase listings (frontend and backend)
- Added order history
- Users can delete accounts, change their password, and update settings
- Users can edit all listing properties
- Added working realtime chat between buyers and sellers. 
...
Generates a one-time token for the user to reset their password.
- Auth: No
- Response: A simple reset token

#### POST /reset-password
Resets the user's password
- Auth: No (TODO: Add under auth)
- Response: Requst status (OK or Failed)
...
#### GET /listings/me
Fetch the current user's available listings
- Auth: Yes
- Response: List of listings
...
#### PUT /listings/id
Update a listing given by the listing ID
- Auth: Yes
...
- Auth: Yes
- Body: Order data (listingID, buyerID, sellerID, etc.)
...
#### GET /conversations
Retrieve the current user's conversations
- Auth: Yes
...
Retrieve the messages of a specific conversation by ID
- Auth: Yes
...
#### GET /ws/chat/id
Upgrades an HTTP request to a Websocket for realtime chatting
- Auth: Yes
...
- Listing Service:
 ...
 - User Service:
  - TestGetAll_ReturnsUsers: Pass (Returns at least the seeded test user)
  - TestGetByID_ExistingUser: Pass (Returns correct user given a valid ID)
  - TestGetByID_NotFound: Pass (Returns err for unknown ID)
  - TestGetByEmail_ExistingUser: Pass (Returns correct user given a valid email)
  - TestGetByEmail_NotFound: Pass (Returns err for unknown email)
  - TestCreate_Success: Pass (Creates user, assigns UUID, and stores password as bcrypt hash)
  - TestCreate_DuplicateEmail: Pass (Returns err when email is already registered)
  - TestDelete_ExistingUser: Pass (Deletes user and confirms it is no longer retrievable)
  - TestDelete_NotFound: Pass (Returns gorm.ErrRecordNotFound for unknown ID)
  - TestUpdate_Success: Pass (Updates first and last name correctly)
  - TestUpdate_NotFound: Pass (Returns gorm.ErrRecordNotFound for unknown ID)
  - TestUpdateProfileImage_Create: Pass (Creates profile image and returns a valid UUID)
  - TestUpdateProfileImage_Update: Pass (Updates existing profile image and returns the same UUID)   
- Order Service:
  - TestOrderServiceCreateFromListingSuccess: Pass (Creates order with correct buyer, listing, price, and status; marks listing as sold)
  - TestOrderServiceCreateFromListingMarksAsSold: Pass (Prevents a second order from being created on an already-sold listing)
  - TestOrderServiceGetByBuyerIDOrdering: Pass (Returns buyer's orders sorted by purchased_at descending)
  - TestOrderServiceGetByBuyerIDFilters: Pass (Returns only orders belonging to the specified buyer)
  - TestOrderServiceCreateFromListingPropagatesDBError: Pass (Creates order successfully and confirms listing ID is set correctly)
